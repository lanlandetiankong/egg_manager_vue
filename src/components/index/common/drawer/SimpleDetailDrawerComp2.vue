    ...
    export default {
        ...
        methods: {
            dealGetDataObjKeyValFromEnum(fieldKey,fieldVal,fieldName){  //尝试从enum中取得该值在枚举的映射值，如果没有为该key配置枚举映射则会返回原值
                var keyEnumValMapObj = this.drawerFieldConf.keyEnumValMapObj;
                var undefinedEnumKeyValue = this.drawerFieldConf.undefinedEnumKeyValue;
            ...
            dealDataObjToArr(obj){  //将object转化为展示的数组
                ...
                if(typeof this.drawerFieldConf != "undefined" && this.drawerFieldConf != null){
                    var showKeyArr = this.drawerFieldConf.showKeyArr;
